...
{
    public class PrevDestructibleObject : MonoBehaviour, IHealth
    {
        ...
        private void DestroyMesh()
        {
            ...
            var parts = new List<PrevPartMesh>();
            var subParts = new List<PrevPartMesh>();

            var mainPart = new PrevPartMesh()
            {
                UV = originalMesh.uv,
            ...
            for (var c = 0; c < CutCascades; c++)
            {
                ...
                parts = new List<PrevPartMesh>(subParts);
                subParts.Clear();
            }
            ...
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                Vector3 direction = new Vector3(parts[i].Bounds.center.x, Mathf.Max(parts[i].Bounds.center.y, 0.0f), parts[i].Bounds.center.z);
                parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(direction * ExplodeForce, transform.position, ForceMode.Impulse);
        ...
        private PrevPartMesh GenerateMesh(PrevPartMesh original, Plane plane, bool left)
        {
            var partMesh = new PrevPartMesh() { };
            var ray1 = new Ray();
            var ray2 = new Ray();
        ...
        private void AddEdge(int subMesh, PrevPartMesh partMesh, Vector3 normal, Vector3 vertex1, Vector3 vertex2, Vector2 uv1, Vector2 uv2)
        {
            if (!edgeSet)
            {
                edgeSet = true;
            ...
            public class PrevPartMesh
        {
            ...
            public void MakeGameobject(PrevDestructibleObject original)
            {
